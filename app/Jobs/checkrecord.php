...
class checkrecord implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        //    ->orderby('tanggal', 'desc')->get();
            // for($x=1;$x<$totalRec->count();$x++){
            //     $totalsell = 0;
            //     $totalpurc = 0;
            //     Log::info($x);
            //     $total_sell = Sell::where('id_produk', $produk->id_produk)->whereMonth('tgl_sell', $x)
            //     ->whereYear('tgl_sell', '<=', $year)->get();
            //     $total_purchase = Purchase::where('id_produk', $produk->id_produk)->whereMonth('tgl_purchase', $x)
